...
```{r}
readRDS(sum_path)$mod[[1]]

xl <- seq(10,200,2)
# Get prediction intervals
df_pred <- 
as.data.frame(readRDS(sum_path)$mod[[1]]) %>% 
  rowwise() %>% 
  mutate(sigmaY = list(expPlateau(a1, maxVPD, minVPD, xl)), 
         YIELD = list(quadPlateau(b1, AOPD, xl)),
         STAND = list(xl)
         ) %>% 
...
ggplot()+
  ...
  theme_test()+
  geom_point(data = readRDS(sum_path)$data[[2]], aes(x = STAND, y = YIELD), shape = 21, fill = "#d5d8de", size = 3, alpha = .5)
...
readRDS(sum_path)$mod_sum[[2]] %>% 
ggplot()+
  geom_line(aes(x = STAND, y = pred_q500), size = 1)+
  geom_line(aes(x = STAND, y = pred_q975))+
  geom_line(aes(x = STAND, y = pred_q025))+
  geom_ribbon(aes(x = STAND,ymin = pred_q025, ymax = pred_q975), alpha = 0.3, fill = "red")+
  theme_test()
