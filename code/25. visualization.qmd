...
label <- unique(points2[,c(1,2,4,5,8)]) %>% 
  ungroup() %>% 
  mutate(label = c("G", "H", "E", "F", "A", "B", "C", "D"), 
         # Define variables for facet_wrap
         FW = paste0(TRT, "-", estimate, YE),
         # Roerder facets
         FW = fct_relevel(as.factor(FW),levels =  levels)
...
p4
ggsave(plot = p4 , "../output/plots/figures/Figure 4.png", width = 8, height = 5, dpi=600)
...
```{r}
quantile(sum_df[,c("mod")][[1]][[1]][,2], probs = .99)
quantile(sum_df[,c("mod")][[1]][[1]][,2], probs = .01)

sum(ifelse(sum_df[,c("mod")][[1]][[1]][,5] - sum_df[,c("mod")][[1]][[1]][,2]>0,0,1))/ length(sum_df[,c("mod")][[1]][[1]][,5])
```
